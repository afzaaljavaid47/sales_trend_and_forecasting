    ...
    if(data)
     {
      ...
      PythonShell.run('/Sales_Forecasting.py', options, (err, data)=>{ 
        ...
        if (err)
          ...
          if(data){
            res.send({data1:data[0],data2:data[1],data3:data[2],file:`http://localhost:5000/excel_files/${user_ID}/trend_all.csv`})
...
app.get('/future_sales_data', (req, res)=> { 
  ...
  if(data)
   {
  fs.createReadStream(`./excel_files/${data._id}/Sales_Forecasting_Predicted_file.csv`)
  .pipe(
      parse({
          delimiter:','
      })
  ...
  .on('end',function(){
      console.log(csvData)
      res.status(200).json({data:csvData,file:`http://localhost:5000/excel_files/${data._id}/Sales_Forecasting_Predicted_file.csv`})
  })
}
})
})
